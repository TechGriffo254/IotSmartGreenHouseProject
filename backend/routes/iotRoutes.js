...
router.post("/", async (req, res) => {
  try {
    ...
    if (
      pincode &&
      pincode !== process.env.ESP32_PINCODE &&
      pincode !== "123456"
    ) {
...
router.post("/device-status", async (req, res) => {
  try {
    ...
    console.log(`📤 Device status update from ESP32:`, {
      deviceId,
      status,
      autoMode,
    });
  ...
  try {
    ...
    for (const reading of readings) {
      try {
        ...
        // Emit real-time data
        if (io) {
          io.to(`greenhouse-${sensorData.greenhouseId}`).emit(
            "sensorUpdate",
            sensorData
          );
        }
...
router.post("/old-format", async (req, res) => {
  try {
    ...
    console.log(`📤 Device status update from old format:`, {
      deviceId,
      status,
      autoMode,
    });
...
router.post("/legacy", async (req, res) => {
  try {
    ...
    console.log("📊 Legacy sensor data:", {
      temperature,
      humidity,
      soilMoisture,
      lightIntensity,
...
async function checkComprehensiveThresholds(
  sensors,
  greenhouseId,
  deviceId,
  io
  ...
  for (const alertData of alerts) {
    ...
    console.log(
      `🚨 Alert created: ${alertData.alertType} - ${alertData.message}`
    );
  }
}
...
async function getDeviceCommands(deviceId, greenhouseId) {
  try {
    ...
    const devices = await DeviceControl.find({
      greenhouseId,
      $or: [{ deviceId: "WATER_PUMP_001" }, { deviceId: "WINDOW_SERVO_001" }],
    });
...
async function checkThresholds(sensorData, io) {
  try {
    ...
    if (soilMoisture !== undefined && !isNaN(soilMoisture)) {
      if (
        soilMoisture < (process.env.ALERT_THRESHOLD_SOIL_MOISTURE_LOW || 300)
      ) {
        alerts.push({
          alertType: "SOIL_MOISTURE_LOW",
...
async function ensureIoTDevicesExist(greenhouseId) {
  try {
    ...
    let waterPumpDevice = await DeviceControl.findOne({
    ...
    let windowDevice = await DeviceControl.findOne({
