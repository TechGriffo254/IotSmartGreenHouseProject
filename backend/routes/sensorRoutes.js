...
const { validateSensorData } = require("../backend/middleware/validation");
...
async function checkAndCreateAlerts(sensorData, io) {
  ...
  if (
    ...
    sensorData.soilMoisture < process.env.ALERT_THRESHOLD_SOIL_MOISTURE_LOW
  ) {
    alerts.push({
      greenhouseId: sensorData.greenhouseId,
  ...
  if (
    ...
    sensorData.lightIntensity < process.env.ALERT_THRESHOLD_LIGHT_LOW
  ) {
    alerts.push({
      greenhouseId: sensorData.greenhouseId,
