...
const DeviceControlLog = require("../backend/models/DeviceControlLog");
const { auth } = require("../backend/middleware/auth");
const { validateDeviceControl } = require("../backend/middleware/validation");
...
router.delete("/:deviceId", auth, async (req, res) => {
  try {
    ...
    const io = req.app.get("io");
    io.to(`greenhouse-${device.greenhouseId}`).emit("deviceRemoved", {
      deviceId,
    });
...
router.post("/:deviceId/control", auth, async (req, res) => {
  try {
    ...
    console.log(
      `🎛️ Manual control request for device: ${deviceId}, action: ${action}`
    );
    ...
    switch (action) {
      ...
      case "set_auto_mode":
        ...
        message = `${device.deviceName} auto mode ${
          device.autoMode ? "enabled" : "disabled"
        }`;
        break;
      default:
...
router.get("/control-history/:greenhouseId", auth, async (req, res) => {
  try {
    ...
    const history = await DeviceControlLog.getControlHistory(
      filters,
      parseInt(limit)
    );
...
router.post("/setup-iot-devices", auth, async (req, res) => {
  try {
    ...
    let windowServo = await DeviceControl.findOne({
